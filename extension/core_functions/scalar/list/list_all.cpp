...
static unique_ptr<FunctionData> ListAllBind(ClientContext &context, ScalarFunction &bound_function,
...
static LogicalType ListAllBindLambda(ClientContext &context, const vector<LogicalType> &function_child_types,
                                     const idx_t parameter_idx) {
	return LambdaFunctions::BindBinaryChildren(function_child_types, parameter_idx);
}

ScalarFunction ListAllFun::GetFunction() {
	ScalarFunction fun({LogicalType::LIST(LogicalType::ANY), LogicalType::LAMBDA}, LogicalType::BOOLEAN,
	                   LambdaFunctions::ListAllFunction, ListAllBind, nullptr, nullptr);
	...
	fun.SetBindLambdaCallback(ListAllBindLambda);
